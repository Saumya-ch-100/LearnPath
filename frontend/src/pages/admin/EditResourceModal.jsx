...
const EditResourceModal = ({ resource, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    fetchSkills();
    if (resource) {
      // Handle skillIds - they might be populated objects or just IDs
      const skillIds = resource.skillIds 
        ? resource.skillIds.map(skill => typeof skill === 'string' ? skill : skill._id)
        : [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axiosClient.patch(`/resources/${resource._id}`, payload);
      onSuccess();
      onClose();
    } catch (error) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl my-8">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Edit size={24} className="text-purple" strokeWidth={2.5} />
            Edit Resource
          </h2>
          <button
            onClick={onClose}
